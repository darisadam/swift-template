...
> 🎨 **SwiftUI only.** Every template uses SwiftUI views, `@Observable` view models, and `async/await` concurrency. **UIKit, Storyboards, and XIBs are not supported** — if your app must use those, this isn't the right starting point. Fork and adapt, or pick a different toolkit.
...
<details>
<summary><b>Can I use UIKit with these templates?</b></summary>

**No.** Every template is SwiftUI-only. ViewModels are `@Observable @MainActor`, views are SwiftUI `View` structs, and navigation uses `NavigationStack` / TCA's `Scope`. There's no `UIViewController`, no Storyboard, no XIB anywhere in the source.

If your app must use UIKit (large existing UIKit codebase, third-party UIKit-only libraries, etc.), fork a template and adapt it — but you'll be rewriting most of the view layer, and the per-template "Try this first" exercises won't apply. You'd be better off starting from Xcode's UIKit App template instead.

SwiftUI ↔ UIKit interop via `UIViewRepresentable` / `UIHostingController` is fine and common — you can embed UIKit views in a SwiftUI app generated by this tool. The constraint is only at the **app architecture** level.
</details>
